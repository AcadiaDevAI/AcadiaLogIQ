...
def invoke_json(self, *, system: str, prompt: str, max_tokens: Optional[int] = None) -> Optional[Dict[str, Any]]:
    ...
    for attempt in range(1, settings.MAX_METADATA_RETRIES + 1):
        try:
            ...
            try:
                ...
                logger.warning("Haiku returned invalid JSON: %s | raw=%s", json_exc, raw[:1200])
                return None
